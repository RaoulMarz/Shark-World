...
class GameMenuScreen extends BaseScreen {
  ...
  override def initialize(): Unit = {
    ...
    clearBagItems()
    clearMusicPlayers()
    //music_Intro = Gdx.audio.newMusic(Gdx.files.internal(assetMusicPath + "/game_intro.ogg"))
    //addMusicPlayer("menu-intro", music_Intro)
    ...
    //scene.addSegment(new SceneSegment(sharkProwler, SceneActions.moveToScreenCenter(2)))
  ...
  def showGameTitle() = {
    if (dialogTitleBox == null) {
      ...
      //dialogTitleBox.setPosition(appWidth / 2.8f, appHeight / 3.9f)
  ...
  def showGameHelpInfo(): Unit = {
    if (dialogInfoHelp == null) {
      ...
      //dialogInfoHelp.setPosition(20.0f, appHeight - 25.0f)
  ...
  def menuActionEvent(keyCode: Int): Unit = {
    if (menuMode) {
      val menuSelectedValue = getBagItem("menu_selected_item")
      menuSelectedValue match {
