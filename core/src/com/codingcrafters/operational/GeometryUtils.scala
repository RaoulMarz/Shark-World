...
object GeometryUtils {
  ...
  def getExtendedPoint(sourceCoordinate : Position, targetCoordinate : Position, lineDistance : Int) : Position = {
    ...
    if (gradient != Double.NaN) {
      if (gradient == 0.0) {
        ...
        val extendVector : Position = Position((lineDistance * normalVector._1).toInt, (lineDistance * normalVector._2).toInt)
  ...
  def ellipsoidAreaCoordinates(center: Position, radiusX : Float, radiusY : Float, width : Int, height : Int) : List[Position] = {
    ...
    val endX = Math.min(width, center.x + radiusX)
    val endY = Math.min(width, center.y + radiusY)
    /*
    for (
      i <- Range(startX, endX);
      j <- Range(startY, endY)
