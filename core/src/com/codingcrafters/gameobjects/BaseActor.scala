...
object BaseActor { // stores size of game world for all actors
   ...
   */
  def count(stage: Stage, className: String): Int = getList(stage, className).size
}
...
class BaseActor(val xP: Float, val yP: Float, val s: Stage) extends Group { // call constructor from Actor class
   ...
   */
  def loadAnimationFromFiles(fileNames: scala.Array[String], frameDuration: Float, loop: Boolean): Animation[TextureRegion] = {
    ...
    if (fileCount > 0) {
      ...
      frameWidth = textureTmp.getWidth
      frameHeight = textureTmp.getHeight
  ...
  def loadAnimationFromSheetScaled(fileName: String, rows: Int, cols: Int, frameDuration: Float, loop: Boolean, scaleX : Float, scaleY : Float, totalDuration : Float = 0.0f): Animation[TextureRegion] = {
    val texture = new Texture(Gdx.files.internal(fileName), true)
    texture.setFilter(TextureFilter.Linear, TextureFilter.Linear)
    setScaleX(scaleX)
    setScaleY(scaleY)
   ...
   */
  def getMotionAngle: Float = velocityVec.angle
  ...
  def boundToWorld(): Unit = {
    if (getX < 0) setX(0)
    if (getX + getWidth > BaseActor.worldBounds.width) setX(BaseActor.worldBounds.width - getWidth)
    if (getY < 0) setY(0)
    if (getY + getHeight > BaseActor.worldBounds.height) setY(BaseActor.worldBounds.height - getHeight)
  }
  ...
  def alignCamera(): Unit = {
    ...
    cam.position.x = MathUtils.clamp(cam.position.x, cam.viewportWidth / 2, BaseActor.worldBounds.width - cam.viewportWidth / 2)
    cam.position.y = MathUtils.clamp(cam.position.y, cam.viewportHeight / 2, BaseActor.worldBounds.height - cam.viewportHeight / 2)
    cam.update()
  }
  ...
  override def draw(batch: Batch, parentAlpha: Float): Unit = { // apply color tint effect
    ...
    if (animation != null && isVisible) {
      var offsetX = 0.0f
      var offsetY = 0.0f
      if (adjustScalingTranslation && (frameWidth >= 2.0f) && (!GeometryUtils.withinTolerance(getScaleX, 1.0f, 0.03f)) )  {
        offsetX = (1.0f - getScaleX) * frameWidth
        offsetY = (1.0f - getScaleY) * frameHeight
      }
      batch.draw(animation.getKeyFrame(elapsedTime), getX - offsetX, getY - offsetY, getOriginX, getOriginY, getWidth, getHeight, getScaleX, getScaleY, getRotation)
    }
